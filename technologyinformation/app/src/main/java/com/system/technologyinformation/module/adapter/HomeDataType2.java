...
public class HomeDataType2 extends MultiItemView<HomeDataBean> {
    Context context;

    public HomeDataType2(Context context) {
    ...
    public int getLayoutId() {
        return R.layout.adapter_home_data_type2;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, @NonNull HomeDataBean homeDataBean, int i) {
        ...
        if (homeImg!=null&&homeImg.size() >= 3) {
            ImageView tv_homeimg1 = viewHolder.getView(R.id.tv_homeimg1);
            Glide.with(context).load(homeImg.get(0)).into(tv_homeimg1);
            Log.i("tv_homeimg1",homeImg.get(0));
            ImageView tv_homeimg2 = viewHolder.getView(R.id.tv_homeimg2);
            Glide.with(context).load(homeImg.get(1)).into(tv_homeimg2);
            Log.i("tv_homeimg2",homeImg.get(1));
            ImageView tv_homeimg3 = viewHolder.getView(R.id.tv_homeimg3);
            Glide.with(context).load(homeImg.get(2)).into(tv_homeimg3);
            Log.i("tv_homeimg3",homeImg.get(2));
        }
