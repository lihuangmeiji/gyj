...
import com.system.technologyinformation.module.contract.ForgotPasswordContract;
import com.system.technologyinformation.module.presenter.ForgotPasswordPresenter;
...
public class ForgotPasswordActivity extends BaseActivity<ForgotPasswordPresenter> implements ForgotPasswordContract.View {
    ...
    @BindView(R.id.tv_fg_password)
    TextView tv_fg_password;
    ...
    protected void initEventAndData() {
        ...
        Observable.combineLatest(phoneObservable, codeObservable, pwdObservable, new Function3<CharSequence, CharSequence, CharSequence, Boolean>() {
            ...
            public void accept(Boolean aBoolean) throws Exception {
                tv_fg_password.setEnabled(!aBoolean);
                tv_fg_password.setSelected(!aBoolean);
            }
        });
    ...
    @OnClick({R.id.tv_fg_password, R.id.tv_get_code, R.id.tv_close})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.tv_fg_password:
                getPresenter().forgotPasswordInfo(et_phone.getText().toString().trim(),et_pwd.getText().toString(),et_code.getText().toString().trim());
                break;
            case R.id.tv_get_code:
                getPresenter().forgotPasswordCheckPhone(et_phone.getText().toString().trim());
                break;
            case R.id.tv_close:
                finish();
                break;
    ...
    @Override
    public void forgotPasswordInfoResult(BaseResponseInfo baseResponseInfo) {
    ...
    @Override
    public void forgotPasswordSmsResult(BaseResponseInfo baseResponseInfo) {
        if(baseResponseInfo.getCode()==0){
    ...
    @Override
    public void forgotPasswordCheckPhoneResult(BaseResponseInfo baseResponseInfo) {
        if(baseResponseInfo.getCode()==0){
            boolean cp= Boolean.valueOf(baseResponseInfo.getData().toString());
            if(cp){
                toast("该手机号未注册");
            }else{
                getPresenter().forgotPasswordSms(et_phone.getText().toString().trim(),"1");
            }
        }else{
            toast(baseResponseInfo.getDetail());
        }
