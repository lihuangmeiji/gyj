...
public class IndicatorView extends View {
    ...
    public IndicatorView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            indicatorColor = typedArray.getColor(R.styleable.IndicatorView_indicatorColor, getContext().getResources().getColor(R.color.tabwd));
            indicatorColorSelected = typedArray.getColor(R.styleable.IndicatorView_indicatorColorSelected, getContext().getResources().getColor(R.color.color37));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (indicatorCount > 0) {
            for (int i = 0; i < indicatorCount; i++) {
                ...
                switch (gravity) {
                    case 0:
                        left = (viewWidth - totalWidth) / 2 + (i * 2 * indicatorWidth*2);
                        break;
                    case 1:
                ...
                int right = left + indicatorWidth*2;
                int bottom = top + indicatorWidth;
                //80, 260, 200, 300  RectF(370.0, 26.0, 382.0, 38.0)
